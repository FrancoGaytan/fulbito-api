...
const Out = z.object({
  teams: z.array(z.object({ name: z.string(), players: z.array(z.string()) })).length(2),
});
export type AiTeams = z.infer<typeof Out>;

export async function suggestTeamsWithGemini(input: {
  participants: Array<{ id: string; name: string; rating: number; abilities: Record<string, number> }>;
  seed?: number;
}): Promise<AiTeams> {
